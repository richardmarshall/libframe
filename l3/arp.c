...
header_t *arp_header_create( uint16_t htype, uint16_t ptype, uint8_t hsize, uint8_t psize, uint16_t oper, uint8_t *sha, uint8_t *spa, uint8_t *tha, uint8_t *tpa ) {
...
/* create new arp pdu */
pdu_t *arp_create( uint16_t htype, uint16_t ptype, uint8_t hsize, uint8_t psize, uint16_t oper, uint8_t *sha, uint8_t *spa, uint8_t *tha, uint8_t *tpa ) {
	pdu_t *pdu = NULL;
	header_t *header = NULL;

	/* create arp header */
	header = arp_header_create(htype,ptype,hsize,psize,oper,sha,spa,tha,tpa);
...
pdu_t *arp_ie_simple_create( uint16_t oper, char *sha, char *spa, char *tha, char *tpa ) {
	uint8_t bsha[6], btha[6];
	uint8_t bspa[4], btpa[4];

	/* convert strings */
	if (!parse_mac_string(sha, bsha)) return NULL;
	if (!parse_mac_string(tha, btha)) return NULL;
	if (!parse_ip_string(spa, bspa)) return NULL;
	if (!parse_ip_string(tpa, btpa)) return NULL;

	/* return results of arp_create */
	return arp_create(ARP_HTYPE_ETHERNET,ETYPE_IP,6,4,oper,bsha,bspa,btha,btpa);
}
